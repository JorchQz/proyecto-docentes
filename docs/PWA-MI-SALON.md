...
- **"Propio" es lo que este dispositivo envió, desde cualquier ventana (2026-09-25, tras R12).** R12 encontró dos carreras en el MISMO aparato que daban avisos falsos y descartaban la última corrección: un doble toque mientras se refrescaba el token con la respuesta perdida, y dos ventanas de Hoy (la app instalada y una pestaña) que comparten la cola, cuando la otra ventana envía lo que capturó esta.
  - Justo antes de enviar, el valor de cada captura se anota en IndexedDB (almacén `propias` de `jissez-bandeja`, por llave de dato, con la cuenta dueña). Cualquier ventana lo reconoce como propio, aunque lo haya enviado otra ventana o la respuesta se haya perdido. Al escribir, además de la versión que vio la pantalla y lo heredado, cuentan como propios los valores anotados por capturas **anteriores** de este dispositivo.
  - El orden es el de captura en el dispositivo: un contador en ese mismo almacén (`#orden`), compartido por todas las ventanas. **No es el reloj** (un reloj adelantado que luego se corrige no desordena nada; ver r23 d y e).
  - Si ya salió una captura **más nueva** de la misma llave, la vieja no se escribe (queda "superada", sin aviso): nunca una captura vieja pisa a una más nueva del mismo aparato.
  - Antes de enviar se vuelve a leer la cola después de comprobar la sesión (un refresco del token con poca señal puede tardar): se envía lo último que tocó la maestra.
  - Se olvida lo que ya no hace falta: al confirmarse una captura, lo anotado antes de ella; tras un conflicto, esa captura y las anteriores; tras un rechazo, la suya; y al abrir, lo anotado hace más de 7 días.
  - **Un solo aviso por dato.** Si la maestra tocó varias veces el mismo dato antes del aviso de conflicto (todas sobre la misma vista vieja), ninguna se aplica y se avisa una vez, con su último toque. Lo que toque después del aviso ya parte de lo que hay en la base y se escribe normalmente. En Hoy, un aviso nuevo del mismo dato reemplaza al anterior.
  - Entre aparatos no cambia nada: un valor que puso otro aparato y que este no envió (o ya olvidó) sigue siendo conflicto.
  - **Límite que queda:** si otro aparato pone justo un valor que este anotó y aún recuerda (el último que confirmó o uno en camino), no se distingue del propio y no se avisa (el dato queda como lo decidió la última captura de este aparato). Con la regla por contenido, eso ya pasaba con la versión que vio la pantalla.
  - Las dos ventanas no se avisan entre sí (no hay BroadcastChannel): cada una pinta lo suyo; la regla de arriba basta para que ninguna dé avisos falsos ni pise a la otra. Si las dos envían la misma calificación nueva a la vez, una puede dejar un 409 en la consola (inofensivo: la otra ya la guardó).
...
- **Lo que no se guardó se ve donde esté la maestra** (tras R12): además de la lista de arriba, un aviso fijo abajo (`role="alert"`, en la misma pila que el estado de la cola, por encima de la barra de secciones en celular) con el último dato que no se guardó, "Ver detalle" (lleva a la lista y le pone el foco) y "Cerrar". "Entendido" en la lista quita los dos.
- **Fuera de Hoy** (las páginas de la barra que cargan `js/bandeja-salida.js`): si la cuenta de la sesión guardada tiene capturas de Hoy sin enviar, un aviso discreto abajo lo dice ("Tienes N capturas de Hoy sin enviar." y si no hay señal, que se enviarán solas al volver) con el botón "Ir a Hoy"; con red se envían en segundo plano con la misma regla. Lo que no se aplicó (conflicto o rechazo) se dice en ese aviso, con "Entendido". Sin capturas pendientes no se abre ni se crea nada; en Hoy no corre (tiene la suya). Usa el auth sin envolver: una falla de red al comprobar la sesión no detiene la página.
...
- **Cerrar sesión con capturas pendientes** (menú de la cuenta y Sala de Maestros; `js/bandeja-salida.js` se carga en esas páginas): pide confirmar ("Tienes N capturas de Hoy sin enviar. Se quedan guardadas en este dispositivo y se enviarán cuando vuelvas a entrar en él con señal. ¿Cerrar sesión de todos modos?").
...
- Carreras del mismo aparato (2026-09-25, `.qa/constructor-r-propias/`, grupo propio "R prop Multigrado" en QA2, con los guiones de R12; `ANTERIOR=1` sirve el código de 3d48d1a): `r91` (doble toque con token vencido y respuesta perdida; anterior: queda Falta y aviso falso; nuevo: queda Justificada, sin aviso), `r93` (la otra pestaña envía; anterior: la corrección se descarta con dos avisos falsos; nuevo: se aplica), `r92` (dos pestañas con dobles toques; anterior: aviso falso del cierre retirado; nuevo: sin avisos y siempre el último toque), `r95` (1280 y 390: dos toques antes del aviso dan un solo aviso, visible abajo aunque la maestra esté en el Cierre del día; el toque después del aviso se escribe), `r94`/`r96` (fuera de Hoy: aviso con "Ir a Hoy" y envío en segundo plano, con un conflicto avisado ahí mismo). Sin cambios en lo de antes: `r11-dos.js con|sin|retiro|borrado`, `r23-reloj.js` (a-e), `r10`, `r12`, `r15`, `r17`, `r18`, `.qa/humo.js` y el humo de `/salon/`.
...
- `sin-conexion.html` todavía dice "Se enviarán cuando vuelva la señal y abras Hoy": ahora también se envían desde las demás páginas. Es cierto pero incompleto; cambiarlo obliga a subir `VERSION` de `sw.js`, así que se deja para la siguiente vez que se toque el service worker.
- (Hecho: `_redirects` ya tiene `/salon/index.html /salon/ 301`.)
