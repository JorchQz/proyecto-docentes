document.addEventListener("DOMContentLoaded", async function () {
	...
	function mostrarGaleria() {
		...
		document.addEventListener("keydown", function (e) {
			// Con el visor ampliado abierto, él maneja sus propias teclas.
			if (Tienda.visorAbierto()) { return; }
	...
	function renderPaso1() {
		...
		paso1.innerHTML = orden.map(function (p) {
			...
			return '<button type="button" data-paquete="' + esc(p.id) + '" ' +
				...
				(destacado
					? '<p class="text-[13px] font-bold mt-1.5" style="color:' + Tienda.COLOR_DESCUENTO.texto + '">Ahorras ' + montoCorto(ahorro) + " frente a comprarlos por separado</p>"
					: "") +
		...
		function renderPasoPaquete() {
			...
			pasos[1].innerHTML = opciones.map(function (o) {
				...
				return '<button type="button" data-paq="' + o.clave + '" ' + (o.disp ? "" : 'data-nodisp="1" ') +
					...
					'<p class="text-[13px] text-mute mt-0.5">' + esc(o.detalle) + "</p>" +
					(destacado
						? '<p class="text-[13px] font-bold mt-1.5" style="color:' + Tienda.COLOR_DESCUENTO.texto + '">Ahorras ' + montoCorto(ahorroCiclo) + " frente a comprarlos por separado</p>"
						: "") +
